...
///////////////////////////////////////////////////////////////////////////////////
	...
	public:		
		...
		virtual std::vector<IpAddressesModel> Search(Traverse* traverse)
		{
			return addresses;
		}
	...
	protected:
		...
		IpAddressesModel LAN::Traverse::GetAdaptersAddress()
		{
		...
		void LAN::Traverse::SendICMP(std::string ipAddress)
		{
			...
			ipAddressModel.hostName = ResolveHostname(ipAddress);
			ipAddressModel.macAddress = SendArp(ipAddress);
			ipAddressModel.ipAddress = ipAddress;

			addresses.push_back(ipAddressModel);
		}

		std::string LAN::Traverse::ResolveHostname(std::string ipAddress)
		{
			// Declare and initialize variables
			WSADATA wsaData = { 0 };
			int iResult = 0;
		...
		std::string LAN::Traverse::SendArp(std::string& str_ip)
		{
			DWORD dwRetVal;
			IPAddr DestIp = 0;
		...
		//Members
		//////////
		std::vector<IpAddressesModel> addresses;
	};
...
///////////////////////////////////////////////////////////////////////////////////
	...
	protected:
		...
		virtual std::vector<IpAddressesModel> ThisPcInfo::Search(LAN::Traverse* traverse) override
		{
			IpAddressesModel ipAddressModel;

			ipAddressModel = GetAdaptersAddress();

			SendICMP(ipAddressModel.ipAddress);
			//char* tmpStr = getenv("COMPUTERNAME");
			//std::string pcName(tmpStr);
			//thPool.AddWorker(addresses.size() > 8 ? 8 : addresses.size());

			
			//thPool.enqueue([=] { SendICMP("192.168.5.107"); });
			//thPool.enqueue([=] { SendICMP("192.168.5.30"); });
			//thPool.enqueue([=] { SendICMP("192.168.5.200"); });
			//thPool.enqueue([=] { SendICMP("192.168.5.1"); });
			//thPool.enqueue([=] { SendICMP("192.168.5.122"); });
			//thPool.enqueue([=] { SendICMP("192.168.5.145"); });
